...
DROP TABLE IF EXISTS `member_access_right`;
  ...
  UNIQUE KEY `idxUMemberAction` (`member`,`action`) USING BTREE,
  KEY `idxNDateAssigned` (`date_assigned`) USING BTREE,
  KEY `idxNAction` (`action`) USING BTREE,
...
DROP TABLE IF EXISTS `member_group_access_right`;
  ...
  UNIQUE KEY `idxUMemberGroupAction` (`member_group`,`action`) USING BTREE,
  KEY `idxNAction` (`action`) USING BTREE,
  KEY `idxNDateAssigned` (`date_assigned`) USING BTREE,
